...
REGION="asia-east1"
SERVICE_NAME="firestore-demo-api"
...
build_docker_image() {
  echo ""
  echo -e "${BLUE}[3/8] 建立 Docker 映像...${NC}"
...
configure_docker_auth() {
  echo ""
  echo -e "${BLUE}[5/8] 配置 Docker 認證...${NC}"
...
push_docker_image() {
  echo ""
  echo -e "${BLUE}[6/8] 推送 Docker 映像到 Container Registry...${NC}"
...
deploy_to_cloud_run() {
  echo ""
  echo -e "${BLUE}[7/8] 部署到 Cloud Run...${NC}"
